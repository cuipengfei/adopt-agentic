...
| 声明式放东西 | "往上下文里放内容"、直接描述具体机制（如"写 System Instructions"） |
| 回扣（如"回扣第一原则"） | "回到"、"连接回"——"回扣"在中文里有贿赂含义 |
| 交接便签 | "初始 prompt"、"任务描述"——就是主 Agent 给 Sub Agent 的 prompt，不要造新词 |
...
- **比喻时效性**：涉及 AI/模型能力的比喻，要跟上当前能力水平。"实习生"、"初级程序员"等比喻已过时——当前顶级模型的推理能力远超此水平。比喻要准确反映现实，过时的比喻比没有比喻更误导。
- **标题/小标题禁止文艺化**：标题用直白有信息量的表述，不要修辞包装。"把你的大脑装进 Agent"、"给 Agent 请家教"、"一个司机一个方向盘" 都属于 artsy——读者靠标题定位内容，文艺标题传递不了信息。也不要过度口语化（"并行看人也看活"太 street）。
...
- **概念对比必须基于共同基础**：比较两个概念（如 Command vs Skill）时，先说清共同点（如"都是上下文注入"），再讲差异。禁止通过对比表暗示二者"本质不同"——如果底层机制相同，区别只在触发方式和持续时间，就不该在"本质"行制造虚假对立。
- **技术机制声明需限定来源**：涉及具体 agent 实现细节的断言（如"追加到 System Instructions"），必须加限定词（"在大多数实现中"/"以 X 为例"），不做跨 agent 通用断言——除非有多个 agent 的源码或官方文档支撑。
