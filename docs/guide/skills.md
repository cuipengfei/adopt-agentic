...
有一个常见信号：你在同一个项目里，第三次用自然语言给 agent 解释同一套规则。

靠口头重申不是办法。每次都要说，每次都占上下文；后半段对话里那些规则可能已经被压缩掉了，agent 行为跑偏不是它不听话，是那条规则已经不在上下文里了。

看三个维度判断。

**重复频率。** 偶发一次的操作不值得封装。出现三次以上、每次都要重新叙述的，才有沉淀成 skill 的价值。

**"怎么做"还是"做什么"。** Skill 封装的是持续有效的行为模式，不是一次具体动作。"帮我做一次 review"是"做什么"，说一次就够。"review 时始终检查错误路径和类型安全"是"怎么做"，写成 skill 后加载一次，后续每轮都带上。如果规则在今天整个会话里都需要，写成 skill；只需要这一次，说一句话就行。

**复用边界。** 只对当前项目成立的约定，放进项目级规则文件就行。跨项目通用的规则，比如"commit 消息写法"或"Go 错误处理风格"，才值得封装成可复用的 skill。

粒度也很重要。一个 skill 覆盖太宽，等于往上下文里倒了一本书，大部分内容当前任务用不到。按内聚领域拆细，按需加载，比一个大而全的 skill 有效。反过来，一条规则单独写成一个 skill 也没意义，并进已有的 skill 就行。
