...
把几千行 API 定义塞进 `AGENTS.md`，等于逼程序员背字典。把“禁止 eval”这种铁律丢在某个角落文档里，等于把“禁止酒驾”的牌子挂在酒吧厕所。都没用。
...
**规则债**
- 规则只增不减。
- 半年前加的规则，和上周加的规则，可能早就在打架。
- Agent 不会抱怨指令矛盾，它只会随便挑一个执行。结果看运气。

**审计**
- 把指令文件和 Skills 当代码一样管理，做审查（review），走拉取请求（PR）。
