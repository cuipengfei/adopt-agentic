...
部分 Agent 工具的 hooks 机制允许你在「工具返回」阶段做预处理：拦截原始输出、按格式过滤、只留关键字段，再放进 context。这就是失败日志进入下一轮之前的「摘要/筛选入口」。一个实用输出可以长这样：`{status, exit_code, failing_command, first_error, relevant_tail}`。

另一条思路：**diff-first，不是 diff-only**。如果 hook 或编排层能控制输入，而上一轮已经传过这个文件的完整内容，这一轮优先传 diff；需要判断意图、接口或上下文时，再补相关片段。避免同一份信息在 messages 里重复出现三次。
