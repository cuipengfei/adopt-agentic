...
先说清楚：这一步发生在 `messages` 数组里，不是 system prompt。每次工具调用失败、命令报错、测试不通过，Agent 收到的错误输出都会作为 `tool_result` 追加到对话历史。LLM 在下一轮读到这些内容，才能决定怎么调整。

问题在于：**失败信息如果不加处理地全量塞进去，很快就会成为上下文噪音。**

一次完整的 npm install 错误日志可能很长。全塞进 messages，后续每轮都要带着它滚动——有效推理空间被错误回滚压缩，真正需要的任务信息反而被稀释。
...
| **中**：截断 + 关键段 | 保留错误类型、第一条报错、堆栈顶部 | 多数情况下的合理默认值 |
| **细**：全量日志 | 原样传入，不截断 | 调试复杂、LLM 需要完整上下文才能定位 |

多数 Agent 工具的 hooks 机制允许你在「工具返回」阶段做预处理——拦截原始输出、按格式过滤、只留关键字段，再放进 context。这就是失败日志进入下一轮之前的「净化入口」。

另一条思路：**只传差异，不传全量**。如果上一轮已经传过这个文件的完整内容，这一轮只传「哪一行改了」就够了。避免同一份信息在 messages 里重复出现三次。

范围控制同样关键。不是所有失败都值得回流。一个 lint 警告不需要进入 messages——它应该在 hook 里被判断：严重程度低，直接丢弃或只记到本地日志。只有阻塞执行、需要 LLM 决策的失败，才值得消耗上下文空间。
## 怎么选
