...
::: warning 资料放进去，不等于重点就自动对了
把一大堆文档塞进上下文，只能保证 Agent 看得见，不能保证它抓得住当前任务真正依赖的是哪部分。

喂材料之外，最好顺手点明：这次决策主要依据哪几个文件、哪几段、哪条规则。否则它看到的是一堆信息，不是清楚的优先级。
:::

别把完整的 API 文档、数据库 schema 全塞进 `AGENTS.md`。你应该只给个索引，比如“数据库结构见 `docs/db-schema.md`”。

等它真要写 SQL 了，它会自己去查。让它自己拉取知识，别硬推。
...
## 失败日志怎么变成下一轮的 context

知识喂养不只是你主动把文档、规范、规则推给 Agent。还有一路是 **Agent 工作中产生的失败信息**，如何自动回流进下一轮请求。

先说清楚：这一步发生在对话历史里，不是 system prompt。每次工具调用失败、命令报错、测试不通过，Agent 收到的错误输出都会作为工具结果追加到后续请求。不同 API 形态不一样：OpenAI 常见做法是追加 `role: "tool"` message；Claude 常见做法是在 `user` message 里放 `tool_result` block。LLM 在下一轮读到这些内容，才能决定怎么调整。

问题在于：**失败信息如果不加处理地全量塞进去，很快就会成为上下文噪音。**

一次完整的依赖安装错误日志可能很长。全塞进 messages，后续每轮都要带着它滚动。旧错误日志挤占上下文，真正需要的任务信息反而被稀释。

怎么控制：

| 采集粒度 | 做法 | 适合场景 |
| :------- | :--- | :------- |
| **粗**：只保留最终状态 | 只传「失败 / 成功」和一行摘要 | 简单工具调用、不需要 LLM 自己分析原因 |
| **中**：截断 + 关键段 | 保留错误类型、第一条报错、堆栈顶部，以及日志末尾的 summary | 多数情况下的合理默认值 |
| **细**：全量日志 | 原样传入，不截断；长日志优先保存到文件，只把路径和摘要放进 context | 例外情况：调试复杂、LLM 需要完整上下文才能定位 |

部分 Agent 工具的 hooks 机制允许你在「工具返回」阶段做预处理：拦截原始输出、按格式过滤、只留关键字段，再放进 context。这就是失败日志进入下一轮之前的「摘要/筛选入口」。一个实用的输出格式示例如下：`{status, exit_code, failing_command, first_error, relevant_tail}`。

另一条思路：**diff-first，不是 diff-only**。如果 hook 或编排层能控制输入，而上一轮已经传过这个文件的完整内容，这一轮优先传 diff；需要判断意图、接口或上下文时，再补相关片段。避免同一份信息在 messages 里反复出现。

范围控制同样关键。不是所有失败都值得回流。低风险 lint 警告通常不需要进入 messages，但要先判断四件事：是否阻塞执行，是否需要改计划，是否包含新事实，是否重复出现。四项都是否，就可以在 hook 里丢弃或只记到本地日志。只有阻塞执行、需要 LLM 决策的失败，才值得消耗上下文空间。
