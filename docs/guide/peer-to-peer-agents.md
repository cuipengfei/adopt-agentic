...
**事实上，截至目前，严格意义上支持 Agent 间双向消息（不是 orchestrator → worker 的单向委派，而是 agent A 直接给 agent B 发消息、B 可以回复挑战 A）的产品屈指可数。** 大部分产品——包括许多标榜"多 Agent"的工具——实际上是并行执行（各干各的不互相通信）或层级委派（上级指挥下级）。从"并行执行"到"团队协作"这一步，比想象中难得多。
