...
有一个 bug 要修。某个 Mermaid 图在生产构建里渲染出来，但浏览器里右侧标注被截断，本地预览看不出来。这种只在构建产物里才现身的问题，agent 不容易自己发现。

任务派下去时，DoD 是具体写出来的：`bun run docs:build` 零错误，受影响节点的 Mermaid 渲染结果在截图里能看清所有标注，git diff 只动这一个文件。没有「修好就行」这种说法——agent 不知道「好」长什么样，但它知道怎么跑命令、看输出、截图对比。

第一轮改完，build 通过了。按旧习惯，这里很容易收工。但 DoD 里还有截图验证——截图拉出来，箭头对了，右侧标注还是截断的。技术验证过了，任务验证没过。

Agent 没有自己决定怎么改，而是把截图和当前状态带回给用户：build 通过了，但这里有个截断，怎么处理？用户看了一眼：「布局重来，改成垂直堆叠。」说完继续做别的，agent 继续改。第二轮，DoD 三条全过。

然后是失败回流这一步。Agent 没有就此收摊，而是顺手说了一句：同样的水平布局在另外两个文件里也用了，可能有相同的截断。一次防御性搜索下去，果然找到了。顺手修掉。

最后才是让这次教训真正生效的操作：把「Mermaid 水平布局在浏览器里会截断，改用垂直或分组布局」写进 CLAUDE.md。这条规则不是预先设计出来的，是这次截断逼出来的。下一个会话的 agent、下一个负责图表的 sub-agent，都会带着它出发，不会再踩同一个坑。harness 就是这么长出来的——CLAUDE.md 和 hooks 是踩坑之后沉淀的东西，不是事先规划好的。

DoD 让「做完了」有标准，不靠感觉。失败回流把教训从单次对话抽出来，变成后续所有 agent 的参照。人审节奏决定哪一步停下确认、哪一步放手。三件事同时运转，不是先后顺序。
## 人——改流程比改内容多
