...
## 把 DoD、失败回流、人审节奏放进同一个工作日

> 对应概念：[Human-in-the-Loop](/guide/human-in-the-loop)、[System Instructions](/guide/system-instructions)

有一个 bug 要修。曾遇到过这类问题：Mermaid 图在生产构建里渲染出来，但浏览器里右侧标注被截断，本地预览看不出来。这种只在构建产物里才现身的问题，如果 DoD 里没有截图验证这一条，agent 通常不会主动发现。

任务派下去时，DoD 是具体写出来的：

- 命令验证：`bun run docs:build` 零错误。
- 视觉验证：受影响节点的 Mermaid 渲染结果在截图里能看清所有标注。
- 变更范围验证：`git diff` 只动目标文件。

没有「修好就行」这种说法。Agent 不知道「好」长什么样，但它知道怎么跑命令、看输出、截图对比。

第一轮改完，build 通过了。按旧习惯，这里很容易收工。但 DoD 里还有截图验证——截图拉出来，箭头对了，右侧标注还是截断的。技术验证过了，任务验证没过。

Agent 没有自己决定怎么改，而是把截图和当前状态反馈给用户：build 通过了，但这里有个截断，怎么处理？这里必须人审，因为布局方案属于取舍决策：横向图更紧凑，垂直堆叠更稳，但页面会变长。用户看了一眼：「布局重来，改成垂直堆叠。」说完继续做别的，agent 继续改。第二轮，DoD 三条全过。

然后是失败回流这一步：把这次截断当成同类问题的线索。Agent 没有就此收摊，而是顺手说了一句：同样的水平布局在其他文件里也用了，可能有相同的截断。一次防御性搜索下去，果然找到了。把同类布局也纳入同一轮验证，再按同一套 DoD 修掉。

最后才是让这次教训真正生效的操作：把「修改 Mermaid 或布局后，必须跑 `bun run docs:build` 检查渲染问题」写进系统指令文件。这条规则不是预先设计出来的，是这次截断逼出来的。后续会话在加载这份指令时，会先看到这条约束；相关 agent 也会受它影响，更不容易踩同一个坑。Harness 就是这么长出来的：系统指令文件和 hooks 会吸收一部分踩坑教训，但不是所有教训都值得进入 harness。

DoD 让「做完了」有标准，不靠感觉。失败回流把教训从单次对话抽出来，变成后续会话和相关 agent 的参照。人审节奏决定哪一步停下确认、哪一步放手。三件事同时运转，不是先后顺序。

再往前走一步，好规则不只是把要求写下来。

它会把任务边界、判断标准、验证信号分开写清楚。这样 agent 读到的不是一团“你自己看着办”，而是一套能执行、能检查、能回报的工作说明。
