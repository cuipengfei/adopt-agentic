...
```mermaid
graph LR
    A["🟢 第 1 轮<br/>system + 用户消息<br/>窗口占用 ~10%"] --> B["🟡 第 10 轮<br/>system + 10 轮历史 + 工具返回<br/>窗口占用 ~60%"]
    B --> C["🔴 第 30 轮<br/>早期消息被截断 ❌<br/>窗口占用 100%"]
```
...
改一个具体函数、修一个精确 bug？只给它需要的几个文件。

喂太多，它反而"看见了但用错"，抄错变量名、漏掉约束、把不相关文件的模式混进来。

做精确修改时，给的信息越多，反而越不准。上下文太大，LLM 的注意力被稀释，它开始从不相关的文件里"借鉴"模式，抄错变量名，漏掉约束。

所以使用 agent 其实有两种不同的工作模式：

- **理解模式**：梳理架构、理清依赖——上下文放开，宽一点反而看得清全貌。
- **修改模式**：改具体函数、修精确 bug——上下文收紧，只给需要的几个文件。
...
比如：你在对话中让 agent 读了 `package.json` 的内容——这是会话状态，对话结束就没了。你在项目根目录写了 `CLAUDE.md` 规定"所有函数必须有 JSDoc 注释"——这是持久上下文，每次新对话 agent 都会读到它。
...
第二类听起来很方便，但有个坑：自动积累的内容你不一定看得到。Agent 记住了一个后来被推翻的决策——三个月后，它基于这条过时记忆给了一个看起来合理、实际错误的建议。过时的记忆和过时的文档一样危险。甚至更危险，因为你可能根本不知道那条记忆还在。
