...
最基本的隔离是读写分离。读操作（`read_file`、`grep`）通常无需确认直接执行；写操作和 shell 命令在触发前会暂停，等用户审批或按配置策略自动放行。这道分界线不是单纯的 UI 设计，而是把“观察世界”和“改变世界”两类行为在执行层分开处理。

执行隔离的深度因工具不同而差异明显。Codex 的 `shell` 工具在沙箱内运行，与宿主文件系统隔开，网络访问默认受限；Gemini CLI 的 Trusted Folders 机制让 `run_shell_command` 只能在指定目录下读写；Claude Code 和 OpenCode 的 `bash` 工具则直接在宿主环境执行，依赖权限策略和交互式确认控制边界，没有进程级沙箱。

这种差异很重要：同样叫“执行 shell 命令”，有沙箱和没沙箱的风险截面完全不同。

权限闸的另一层是策略预设。`allow`（静默放行）、`ask`（每次确认）、`deny`（拒绝执行）三档组合，让你在效率和安全之间找一个当前可接受的平衡点。开发早期通常 `allow` 居多；要跑自动化流水线时，反过来把高风险操作锁到 `deny` 或 `ask`。

两类操作容易被低估：一是 `write_file` 覆盖核心配置文件，二是带管道的 `bash` 命令（比如 `grep ... | xargs rm`）。这类组合在没有沙箱隔离的环境里，LLM 生成了错误参数，操作在你反应过来之前就已落地。
