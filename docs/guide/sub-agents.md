...
很多人用 Sub Agent，是因为「可以并行跑，快一点」。这个理由没错，但只说了一半。

并发只是隔离带来的副产品。隔离本身才是核心——它解决的问题是：主上下文已经很长很脏，某个精确的子任务不应该在这堆噪声里跑。

有四类场景最能说明这一点。

**探索型任务**。让 agent 去调研不熟悉的库、验证一个方案可不可行。这类工作会产生大量中间噪声——尝试、报错、死路。把这些放进主上下文，只会污染后续决策。放进隔离的 Sub Agent，噪声留在里面，主线只拿到结论。

**Review 任务**。代码审查有自己的分析逻辑，跟主任务的实现思路是两条线。混在一起，两者都会互相干扰。上下文隔离出去，review 结果干净，主线不被审查过程带偏。

**粗活任务**。批量重命名、格式化、文件迁移——这些操作会堆出大量工具调用记录，但对主任务几乎没有信息价值。塞进主上下文是在烧 token，放进 Sub Agent 就只收「完成了」或「失败了」。

**长日志任务**。某些工具调用会返回大量输出——测试报告、编译日志、API 响应体。Sub Agent 可以消化这堆东西，再把有效信息带回来。

这四类场景有个共同点：子任务的**执行过程**对主线没有参考价值，只有结果有用。

这里也是本章和下一章的分界线。编排（orchestration）讲的是「有了多个 Sub Agent 之后，怎么组织步骤、怎么传递中间结果、分支结果怎么汇合」。本章先解决更早的一步：**要不要隔离**，以及隔离能给你什么。想清楚这个，再去考虑怎么编排才有意义。
