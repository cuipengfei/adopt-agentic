...
不少介绍 Sub Agent 的文章，会先强调并发：可以同时跑几个任务，快一点。这个理由没错，但只说了一半。

并发只是调度方式，隔离才是上下文管理方式。它解决的常见问题是：主上下文已经很长、噪声很多，某个精确的子任务不适合在这堆噪声里跑。

下面四类场景很典型。

**探索型任务**。让 agent 去调研不熟悉的库、验证一个方案可不可行。这类工作往往会产生大量中间噪声：尝试、报错、死路。把这些全塞进主上下文，容易干扰后续决策。放进隔离的 Sub Agent 前，先约定回来什么：结论、证据、风险，而不是一整段探索过程。

**Review 任务**。代码审查常常也适合隔离。它有自己的分析路径，和主任务的实现思路不总是一条线。混在同一条上下文里，容易互相打断，也容易被实现者的解释带偏。隔离不只是省上下文，也是减少偏见。主线只接收审查结论、证据和建议优先级。

**粗活任务**。批量重命名、格式化、文件迁移，这些操作会堆出大量工具调用记录，而主线通常只需要最终状态、影响范围和异常项。把全过程原样塞进主上下文，往往是在浪费上下文空间。放进 Sub Agent 时，要把风险边界写清楚：返回变更清单、失败项、未处理文件，不要只报一句「完成了」。

**长日志任务**。某些工具调用会返回大量输出：测试报告、编译日志、API 响应体。在支持摘要回传的工具里，Sub Agent 可以消化这堆东西，把错误类型、关键证据和下一步建议提炼回来。重点不是少占空间，而是把噪声压成信号。

这四类场景有个共同点：主线通常不需要消费完整执行过程，只需要结果和必要证据。需要时再下钻，但取决于工具是否保留 trace。

这里也是本章和下一章的分界线。如果问题是「要不要拿出去单独做」，这是隔离；如果问题变成「多个结果之间谁先谁后、谁依赖谁」，那就是编排（orchestration）。本章先解决更早的一步：**要不要隔离**，以及隔离能给你什么。

不要为了并发而并发；先看这段执行过程会不会污染主线。
