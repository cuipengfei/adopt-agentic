...
审批疲劳的根源不是「审批太多」，而是把人的注意力消耗在机器能自己判断的事上。

可执行的 DoD 信号大多不需要人出现：测试跑通、构建成功、lint 无报错、exit code 为零。这些可以自动验证、自动存档。需要人出现的只有三种情况：操作不可逆（删数据、推生产）；涉及组织层面判断（上线时机、打破接口兼容性）；以及异常信号超出预设阈值、需要人来定性。其余的，事后抽样回顾就够。
...
这样设计的目的不是让你「少点按钮」——而是让剩下的每次点击都值得点。审批频率降低之后，你对每次弹窗的反应会更认真，不再是条件反射。

但这里有一个容易被忽略的后果：如果你长期只看高风险节点，对自动通过的操作不闻不问，你对系统的理解会停留在「异常检测」层面。等到漏斗配置本身出了问题——什么算高风险、阈值设得对不对——你可能已经失去判断这件事的能力。

所以抽样回顾不是走过场。每隔一段时间，随机抽几条自动通过的记录看一眼，确认漏斗在过滤正确的东西。这既是对当前决策质量的验证，也是对你自己判断力的维护。你得持续理解这套系统，才能在漏斗失效时发现并修正。

人的注意力是有限的。审批策略的终点，是让这份有限的注意力花在机器确实无法替代的地方。
## 本节小结
