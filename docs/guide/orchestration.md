...
主 agent 把任务传给分 agent 时，不要直接复制整个当前上下文。有些 handoff 实现会默认带上完整 history，但这不等于你应该把所有噪声都交给分支。更稳妥的做法是传一份精简的任务描述：任务目标是什么，哪些约束不能碰，执行所需的最小上下文（相关文件路径、关键变量、错误信息）。

这份描述越精简且完备，分 agent 的执行就越可预测。精简不是删关键条件：约束、信息来源和验收标准不能丢。分 agent 不需要猜测主会话「之前聊过什么」，只需要根据这份描述独立执行。
