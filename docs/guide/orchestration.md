...
主会话决定「把什么任务给谁」，分会话决定「怎么做」。两者之间真正流动的，是两份令牌：一份出，一份回。

**出：输入令牌**

主 agent 把任务传给分 agent，不是把当前上下文复制一份扔过去——那样代价太高，噪声也太多。实际传递的是一份精简的任务描述：任务目标是什么，哪些约束不能碰，执行所需的最小上下文（相关文件路径、关键变量、错误信息）。

这份描述越小、越自包含，分 agent 的执行就越可预测。它不需要猜测主会话「之前聊过什么」，只需要根据这份描述独立执行。

**回：输出令牌**

分 agent 完成任务后，同样不是把完整会话历史还给主 agent。它返回的是一份摘要：做了什么、结果在哪、有什么需要注意。主会话能直接拿来用的，只有这份摘要。

摘要的格式直接影响主 agent 能否正确合并多个分支的结果。两个并行的分 agent 各自返回模糊的自然语言描述，主 agent 在合并时很可能遗漏细节或产生冲突。更好的做法是约定结构：「修改了哪些文件」「测试结果是什么」「遗留了什么问题」——字段明确，合并就机械化。

**合并**

并行分支汇合时，主 agent 拿到的是多份摘要，不是多份完整上下文。它需要把这些摘要整合进自己的上下文，决定下一步。整合不是拼接，而是提取每份摘要里对当前任务有用的部分，其余丢掉。
