...
| :--- | :--- | :--- |
| **存在感** | **默认存在**，贯穿整个会话 | **按需触发**，一次性注入 |
| **作用域** | 全局，影响 Agent 的每一次响应 | 任务级局部生效，非全局常驻 |
...
- **上下文流动**：用户输入 `/command` → Agent 展开为 prompt → 注入 `messages` → LLM 消费并响应。命令注入的内容是任务级的，不会常驻后续会话。
...
下一节看 Skills——命令是一次性注入，Skills 是持续性的行为模式配置。
