...
能写成 command，不代表值得写。判断门槛很简单：你重复做了三次以上、步骤固定、每次都清楚自己想触发这件事。

最典型的候选是**工作流检查点**。代码评审前，你每次都要看 diff、跑 lint、确认测试通过。这套流程不会因项目而变，步骤顺序固定，执行时机明确——你知道自己想做，只是不想每次手动输入一遍。这就是 `/review` 存在的理由。

另一类候选是**语境切换**。从调试模式切到文档写作，或从功能开发切到性能排查，这些切换本身就是认知负担。一个 `/debug-mode` 或 `/doc-mode` 让 agent 知道当前任务的性质，也让你自己明确"现在换了模式"这个动作。

command 的核心价值在于**显式调用**。你主动触发，agent 才执行——而不是 agent 自己判断"现在大概该做这件事了"。这种确定性在高风险操作里格外重要：`/deploy`、`/migrate-db` 这类命令，你不希望 agent 自动触发，你希望自己按那个 `/` 的时候是清醒的、主动的。

和 skill 的边界：skill 是注入型，agent 判断任务性质后自己决定加载哪些规则，从那一刻起持续生效。command 是召唤型，你决定现在执行，执行完就结束。如果一套工作流需要"我来决定什么时候做"，它是 command；如果是"让 agent 在合适时机自己带上这套规则"，它是 skill。

和 hook 的边界：hook 不需要你触发，事件发生时自动执行。不要用 command 替代 hook 能做的事——不要写 `/start-logging` 这种命令，该用 hook 在 session 启动时自动挂载日志逻辑。显式调用适合"我来决定时机"的场景，被动触发适合"只要事件发生就执行"的场景。
