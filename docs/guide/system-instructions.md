...
1. **这类失败会重复出现吗？** 换个任务、换个人、过一段时间，同样的坑还在吗？
2. **规则能泛化吗？** 能写成一条对所有类似任务都成立的指令，而不是「这次任务里的特殊处理」？

两题都是「是」，就写进 system instructions。否则，在对话里纠正完就结束。

| 失败类别 | 是否进 system instructions | 原因 |
| :--- | :--- | :--- |
| Agent 删了不该删的配置文件 | ✅ 是 | 任何任务都可能触发，需要全局护栏 |
| 某次任务的格式输出不对 | ❌ 否 | 一次性偏差，在当次对话纠正即可 |
| 用了已弃用的 API | ✅ 是 | 整个代码库都受影响，一条规则防所有人踩坑 |
| 推理链走偏、分析方向偏了 | ❌ 否 | 推理类失误不可泛化为静态规则 |
| 总是忘记跑 lint 就提交 | ✅ 是 | 工作流缺口，全局规则一次性封住 |

还有一条实操原则：进 system instructions 的规则要精炼。每条规则常驻上下文，塞多了会挤掉推理空间。失败日志里能提炼出一条准确的行为指令，才值得写进去。如果只能描述「上次出了问题」，先留在团队的记录里，等模式清晰了再固化。

这就是 system instructions 和会话内纠错的边界：前者管可预测、可重复的系统性风险；后者管一次性、任务特有的偏差。不该进去的硬塞进去，指令文件会越来越臃肿，规则越来越难维护。
## 本节小结
