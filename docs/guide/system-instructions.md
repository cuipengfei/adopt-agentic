...
- **版本化是底裤。** 指令文件必须进 Git。`git blame` 能告诉你半年前是谁、为什么加了这条规则。不在 Git 里的指令等于幽灵规则。
- **变更必须 Review。** 一行指令的变更，威力不亚于改一行核心代码。一个人加了"永远用 tabs"，另一个人加了"永远用 spaces"，冲突要到下个会话才炸。指令变更应该走 PR，让团队知道"这条规则会影响谁"。
- **项目变，指令跟着变。** 技术栈从 REST 切到 GraphQL，指令里关于 API 的写法就得改。架构从单体拆成微服务，指令也得跟上。这不是"有空再说"，这是"下一个会话就出错"。
