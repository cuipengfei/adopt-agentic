...
Agent 跑短任务，你盯着就行。跑长任务——几十分钟、上百轮工具调用——盯不住，也不该盯。

但放手不等于放任。长时运行的 Agent 循环需要三样东西：知道走了多远、知道什么时候该停、知道什么时候该推倒重来。

### 检查点

长任务最怕跑到一半崩了，从头来过。

检查点就是进度存档。好的 Agent 会在关键节点自动保存状态——改完一组文件后 commit，完成一个子任务后记录进度。你也可以主动要求："每完成一个模块就 commit 一次。"

检查点的本质是**把连续的长任务切成可恢复的段**。崩了？从最近的检查点继续，不用从零开始。

### 停止条件

Agent 不知道什么时候该停。你得告诉它。

最清晰的停止条件是外部信号：测试全过、构建成功、TODO 清单全部勾完。模糊的停止条件——"优化到你觉得够好"——会让 Agent 陷入无限循环，反复微调永不满意。

实操：给 Agent 一份可勾选的 TODO 清单或明确的验收标准。它完成一项勾一项。全部勾完就停。这比"做完了告诉我"可靠得多。

### 何时继续 / 何时重启

长会话不是一直长下去就好。上下文窗口有限，越长的对话里，早期信息被压缩或丢弃的概率越大。

| 信号 | 推荐做法 |
|------|---------|
| 任务连贯、上下文窗口充裕 | 继续当前会话 |
| Agent 开始"忘"之前说过的约束 | 重启新会话，带上关键上下文 |
| 任务主题切换（从前端转到后端） | 重启新会话 |
| 反复犯同一个错误 | 重启新会话，换个思路起步 |

重启不是失败。重启是**上下文减法**——砍掉累积的噪声，带着干净的上下文重新出发。有时候，一个全新的会话比一个污染了的长会话效率高十倍。
