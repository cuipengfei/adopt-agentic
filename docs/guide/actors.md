...
更完整的多轮交互序列图，见[上下文](/guide/context#管好上下文)章节。这里重点不是请求怎么膨胀，而是**三方各自干了什么**——用户发意图、Agent 编排+执行、LLM 推理+请求工具。
...
Agent 跑短任务，你盯着就行。可一旦任务要跑很长时间、频繁调用工具，你盯不住，也不该盯。
