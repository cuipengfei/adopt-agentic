...
System Instructions 里写的规则是声明式的：你告诉 LLM「应该这样做」，剩下的交给它自己判断。LLM 会尽量遵守，但如果任务复杂、上下文干扰大，或者规则本身表述有歧义，它可能悄悄绕开。这不是 bug，是 LLM 的工作方式——它在权衡所有信息后生成最可能的输出，规则只是众多输入之一。

Hook 解决的是另一个层面的问题：**某些事必须发生，无论 LLM 怎么判断**。

比如：

- 每次调用 `rm` 之前必须弹窗确认——这不是「应该做」，是「不做不行」。
- 每个工具调用结果必须写入审计日志——不管 LLM 觉得这次任务重不重要。
- 高权限操作（git push、数据库写入）触发前必须走审批流——绕不过去。

规则靠 LLM 自律，hook 靠代码强制。两者的可靠性等级不同。

这个边界还有第二层含义：**hook 是被动触发的，规则是主动解读的**。

规则需要 LLM 主动读取、理解、应用。上下文一长，早写的规则很容易被遗忘或降权。Hook 不依赖 LLM 的记忆——只要事件发生，代码就跑。工具调用前的 hook 不在乎 LLM 此刻在想什么，它就是会触发。

所以两者的正确用法是：

- **规则文件（System Instructions）**：描述期望行为——语气、格式、编码风格、优先级。LLM 根据这些做判断。
- **Hook**：锁死必要动作——审计、拦截、审批、强制校验。不给 LLM 绕过的空间。

Hook 还有一个规则文件做不到的用途：作为**权限策略的执行点**。你可以在工具调用前的 hook 里实现完整的审批逻辑——读取操作放行、写入操作记录、高风险操作暂停等待人工确认。这套逻辑在 LLM 视线之外运行，LLM 无法通过措辞或推理来绕过它。
